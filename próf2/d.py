...
def print_lists(namelists,boys_list,girls_list):
    ...
    percentile_50_boys, percentile_50_girls = get_50th(boys_list, girls_list, boy_frequency, girl_frequency)
...
def get_50th(boys_list,girls_list,boy_frequency,girl_frequency):
    check_boys, check_girls = 0, 0
    rank_boy, rank_girl = 0, 0 
    boy_50, girl_50 = (boy_frequency // 2), (girl_frequency // 2)
    for i in range(len(boys_list)):
        check_boys += boys_list[i][1]
        if check_boys >= boy_50:
            rank_boy = i+1
            break
    for i in range(len(girls_list)):
        check_girls += girls_list[i][1]
        if check_girls >= girl_50:
            rank_girl = i+1
            break
    
    return rank_boy,rank_girl            
